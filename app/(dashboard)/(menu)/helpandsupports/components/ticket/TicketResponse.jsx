  ...
  return (
    <div className='text-primaryText'>
      ...
      <div className='p-4 md:p-5 lg:p-6 xl:p-7'>
        ...
        <div className='flex max-lg:flex-col max-lg:gap-2 justify-between text-[16px] lg:text-[20px] xl:text-[24px]'>
        ...
        <div className='p-4 grid justify-center items-center max-lg:grid-cols-4 lg:grid-cols-9 lg:grid-cols-[1fr_2.5fr_1fr_1.5fr_1.5fr_1.5fr_1.5fr_1fr_1fr] text-[8px] md:text-[10px] lg:text-[14px] xl:text-[18px] bg-[#032E2F] mt-4'>
          <div className='flex justify-center max-lg:hidden'>Serial No.</div>
          ...
          <div className='flex justify-center max-lg:hidden'>Request Date</div>
          <div className='flex justify-center max-lg:hidden'>status</div>
          <div className='flex justify-center'>Query</div>
          <div className='flex justify-center max-lg:hidden'>Response Date</div>
          <div className='flex justify-center'>Response</div>
        </div>
        {slicedTickets.map((row, index) => {
        ...
        {slicedTickets.length === 0 ? <div className='flex justify-center items-center h-40'>No response available!</div> : ""}
        <ResponseModal
          open={open}
          ticket={selectedTicket}
          onClose={() => setOpen(false)}
        />
        <div className='flex justify-between max-lg:gap-1 p-8 text-[8px] md:text-[10px] lg:text-[14px] xl:text-[18px]'>
          <div className='text-secondaryText'>Showing {slicedTickets.length > 0 ? startIndex : '0'} to {startIndex + slicedTickets.length - 1} of {filteredTickets.length} entries</div>
