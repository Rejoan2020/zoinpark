  ...
  return (
    <div className='text-primaryText max-lg:pb-20'>
      ...
      <div className='p-4 md:p-5 lg:p-6 xl:p-7'>
        ...
        <div className='flex max-lg:flex-col max-lg:gap-2 max-lg:w-full justify-between text-[12px] md:text-[16px] lg:text-[20px] xl:text-[24px]'>
        ...
        <div className='grid grid-cols-4 lg:grid-cols-9 lg:grid-cols-[1fr_2fr_1fr_1.5fr_1.5fr_1.5fr_1.5fr_1fr_1fr] text-[8px] md:text-[10px] lg:text-[14px] xl:text-[18px] bg-[#032E2F] mt-4'>
          <div className='p-4 max-lg:hidden'>Serial No.</div>
          <div className='p-4 flex justify-center items-center'>Staking ID</div>
          <div className='p-4 flex justify-center items-center max-lg:hidden'>Investment Name</div>
          <div className='p-4 flex justify-center items-center max-lg:hidden'>Principal (ZOIN)</div>
          ...
          <div className='p-4 flex justify-center items-center max-lg:hidden'>Investment Start</div>
          <div className='p-4 flex justify-center items-center max-lg:hidden'>Payment Mode</div>
        ...
        {slicedHistory.map((row, index) => {
          ...
          return <Row key={row._id} serial={startIndex + index} id={row._id} name={row.packageName}
            ...
            payment={row.packageName} action={'Withdraw'} status={row.status} total = {filteredHistory.length} />
        })}
